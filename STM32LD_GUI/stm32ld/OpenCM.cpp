...
int OpenCM_Cmd_WriteBytes( char *CmdStr, int len )
{
	...
	written_len = ser_write( stm32_ser_id, (u8 *)CmdStr, len );
...
int OpenCM_Cmd_Init( const char *portname, u32 baud )
{
	...
	else
	{
		...
		return STM32_FAIL_CONNECT_OPENCM;
	}
...
int OpenCM_WriteFlash( p_read_data read_data_func, p_progress progress_func )
{
	...
	while( 1 )
	{
		...
		//-- 데이터 전송
		//
	  	OpenCM_Cmd_WriteBytes( (char *)data, datalen );
	...
	OpenCM_Cmd_WriteBytes( (char *)data, 1 );
	OpenCM_Wait_ms(100);
